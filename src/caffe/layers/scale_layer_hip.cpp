...
void ScaleLayer<Dtype>::Forward_gpu(
  ...
  if (bias_layer_) {
    ...
    hipLaunchKernelGGL(ScaleBiasForward<Dtype>,  // NOLINT_NEXT_LINE(whitespace/operators)
        dim3(CAFFE_GET_BLOCKS(count)), dim3(CAFFE_HIP_NUM_THREADS), 0, 0,
        count, bottom_data, scale_data, bias_data, scale_dim_, inner_dim_,
        top_data);
  } else {
    hipLaunchKernelGGL(ScaleForward<Dtype>,  // NOLINT_NEXT_LINE(whitespace/operators)
        dim3(CAFFE_GET_BLOCKS(count)), dim3(CAFFE_HIP_NUM_THREADS), 0, 0,
