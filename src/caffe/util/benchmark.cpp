...
#ifndef CPU_ONLY
    HIP_CHECK(hipEventDestroy(start_gpu_));
    HIP_CHECK(hipEventDestroy(stop_gpu_));
#else
    NO_GPU;
#endif
  }
...
void Timer::Start() {
  if (!running()) {
    if (Caffe::mode() == Caffe::GPU) {
#ifndef CPU_ONLY
      HIP_CHECK(hipEventRecord(start_gpu_, 0));
#else
      NO_GPU;
#endif
    } else {
...
void Timer::Init() {
  if (!initted()) {
    if (Caffe::mode() == Caffe::GPU) {
#ifndef CPU_ONLY
      HIP_CHECK(hipEventCreate(&start_gpu_));
      HIP_CHECK(hipEventCreate(&stop_gpu_));
#else
      NO_GPU;
#endif
    }
